    ...
    def _forward_cube(self, images, images_raw, combined_features, mask_per_image, depth_maps, ground_maps, features, instances, Ks, im_current_dims, im_scales_ratio, experiment_type, proposal_function):
        ...
        if experiment_type['use_pred_boxes']:
            ...
            for i in range(n_gt):
                ...
                point_cloud_scores = score_point_cloud(torch.from_numpy(points_no_ground).to(pred_cubes.device), pred_cubes[i], K=Ks_scaled_per_box)
                segment_scores = score_segmentation(mask_per_image_cpu[i][0], pred_cubes[i].get_bube_corners(Ks_scaled_per_box, im_shape))
                dim_scores = score_dimensions((prior_dims_mean[i], prior_dims_std[i]), pred_cubes[i].dimensions[0])
        ...
        if experiment_type['output_recall_scores']: # MABO
            return p_info, score_IoU2D, score_seg, score_dim, score_combined, score_random, score_point_c, score_deep, stat_empty_boxes, stats_image, stats_off
...
class ROIHeads_Score(StandardROIHeads):
    ...
    def forward(self, pred_cubes, instances, Ks, im_scales_ratio, combined_features):
        if self.training:
            losses, acc = self._forward_cube(pred_cubes, instances, Ks, im_scales_ratio, combined_features)
            return losses, acc
        else:
            pred_instances, _, _ = self._forward_cube(pred_cubes, instances, Ks, im_scales_ratio, combined_features)
            return pred_instances
