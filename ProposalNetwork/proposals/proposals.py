...
def propose_random_xy(reference_box, depth_image, priors, im_shape, K, number_of_proposals=1, gt_cube=None, ground_normal:np.ndarray=None):
    number_of_instances = len(reference_box)
    ...
    def fun(x,coef):
        return coef[0] * x + coef[1]
    # x
    x = torch.rand(number_of_instances,1000) * 2 - 1
    y = torch.rand(number_of_instances,1000) * 2 - 1
...
def propose_random_xy_patch(reference_box, depth_image, priors, im_shape, K, number_of_proposals=1, gt_cube=None, ground_normal:np.ndarray=None):
    number_of_instances = len(reference_box)
