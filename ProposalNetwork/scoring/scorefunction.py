...
from ProposalNetwork.utils.utils import iou_2d, iou_3d, custom_mapping, mask_ioa
...
def score_segmentation(bube_corners, segmentation_mask):
    ...
    '''
    bube_mask = np.zeros(segmentation_mask.size, dtype=np.uint8)
    ...
    polygon_points = cv2.convexHull(np.array(bube_corners), returnPoints=True)

    cv2.fillPoly(bube_mask, [polygon_points], 1)

    return mask_ioa(segmentation_mask, bube_mask)
